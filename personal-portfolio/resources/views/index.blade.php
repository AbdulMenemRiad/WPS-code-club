      ...
      <div class="header-carousel-item">
        ...
        <div class="carousel-caption">
          <div class="carousel-caption-content p-3" style="max-width: 900px">
            ...
            <div class="pt-2">
              <a class="btn btn-primary rounded-pill text-white py-3 px-5 m-2 wow fadeInLeft" data-wow-delay="0.1s" href="{{ url('/projects') }}">{{ __('View My Portfolio') }}</a>
              <a class="btn btn-secondary rounded-pill text-white py-3 px-5 m-2 wow fadeInRight" data-wow-delay="0.3s" href="{{ url('/contact') }}">{{ __('Contact Me') }}</a>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="header-carousel-item">
        ...
        <div class="carousel-caption">
          <div class="carousel-caption-content p-3" style="max-width: 900px">
            ...
            <div class="pt-2">
              <a class="btn btn-primary rounded-pill text-white py-3 px-5 m-2" href="{{ url('/projects') }}">{{ __('View My Portfolio') }}</a>
              <a class="btn btn-secondary rounded-pill text-white py-3 px-5 m-2" href="{{ url('/about') }}">{{ __('My Experience') }}</a>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="row g-5 align-items-center">
          <div class="col-lg-5 wow fadeInLeft" data-wow-delay="0.1s">
            <div class="border bg-secondary rounded shadow-sm overflow-hidden position-relative">
              <img src="{{ asset('img/me3.png') }}" class="img-fluid w-100 rounded" alt="Abdul Meneam Riad" style="min-height: 450px; object-fit: cover;" />
            </div>
          </div>
          <div class="col-lg-7 wow fadeInRight" data-wow-delay="0.3s">
            ...
            <div class="d-flex gap-3">
                <a class="btn btn-primary rounded-pill text-white py-3 px-5" href="{{ url('/about') }}">{{ __('Read Full Story') }}</a>
                <a class="btn btn-outline-primary rounded-pill py-3 px-5" href="{{ asset('resume.pdf') }}" target="_blank"><i class="fas fa-download me-2"></i>{{ __('Download CV') }}</a>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="container py-5">
            <div class="row g-5 align-items-center">
                <div class="col-lg-6 wow fadeInLeft" data-wow-delay="0.1s">
                    ...
                    <p class="mb-4 fs-5">{{ __('Words and code snippets only tell half the story. Watch my recent artificial intelligence and hardware integrations in real-time.') }}</p>
                    <ul class="list-unstyled mb-4 fs-5">
                        <li class="mb-2"><i class="fas fa-check-circle text-primary me-3"></i>{{ __('Computer Vision tracking') }}</li>
                        <li class="mb-2"><i class="fas fa-check-circle text-primary me-3"></i>{{ __('Real-time robotic actuations') }}</li>
                        <li class="mb-2"><i class="fas fa-check-circle text-primary me-3"></i>{{ __('Custom Arduino logic') }}</li>
                    </ul>
                    <a href="{{ url('/projects') }}" class="btn btn-primary rounded-pill py-3 px-5">{{ __('Explore All Hardware') }}</a>
                </div>
      ...
      <div class="container py-5">
        <div class="pb-5">
          <div class="row g-4 align-items-end">
            <div class="col-xl-8">
              <h4 class="text-secondary sub-title fw-bold wow fadeInUp" data-wow-delay="0.1s">{{ __('Featured projects showcase') }}</h4>
            ...
            <div class="col-xl-4 text-xl-end wow fadeInUp" data-wow-delay="0.3s">
              <a class="btn btn-primary rounded-pill text-white py-3 px-5" href="{{ url('/projects') }}">{{ __('View All Projects') }}</a>
            </div>
          </div>
        </div>
        ...
        @if($featuredProjects->count() > 0)
            <div class="training-carousel owl-carousel pt-5 wow fadeInUp" data-wow-delay="0.1s">
              ...
              <div class="training-item bg-white rounded h-100 d-flex flex-column shadow-sm">
                <a href="{{ url('/projects/' . $project->slug) }}" class="training-img rounded-top position-relative d-block">
                ...
                <div class="rounded-bottom border border-top-0 p-4 d-flex flex-column flex-grow-1" style="min-height: 220px;">
                    <a href="{{ url('/projects/' . $project->slug) }}" class="h4 mb-3 d-block text-dark text-decoration-none">{{ $project->title }}</a>
                    <p class="mb-3 flex-grow-1">{{ Str::limit($project->description, 100) }}</p>

                    <div class="mt-auto d-flex justify-content-between align-items-center pt-3">
                        @if($project->demo_url)
                            <a class="btn btn-sm btn-primary rounded-pill text-white py-2 px-3" href="{{ $project->demo_url }}" target="_blank">{{ __('View Demo') }}</a>
                        @elseif($project->live_url)
                            <a class="btn btn-sm btn-primary rounded-pill text-white py-2 px-3" href="{{ $project->live_url }}" target="_blank">{{ __('Live Site') }}</a>
                        @endif
