                            ...
                            <div class="training-img rounded-top position-relative">
                                @if ($project->image_path)
                                    <img src="{{ asset('storage/' . $project->image_path) }}"
                                        class="img-fluid rounded-top w-100" style="height: 250px; object-fit: cover;" />
                                @else
                                ...
                                @endif
                                <span class="bg-primary text-white rounded px-3 py-1 position-absolute"
                                    style="top: 10px; right: 10px;">{{ $project->category }}</span>
                            </div>
                                ...
                                <div class="d-flex gap-2 mt-auto">
                                    @if ($project->demo_url)
                                        <a class="btn btn-outline-primary rounded-pill btn-sm"
                                            href="{{ $project->demo_url }}" target="_blank">Live Demo</a>
                                    @endif
                                    @if ($project->github_url)
                                        <a class="btn btn-dark rounded-pill btn-sm" href="{{ $project->github_url }}"
                                            target="_blank"><i class="fab fa-github me-1"></i> Code</a>
                                    @endif
                                </div>
                            </div>
                        </div>
