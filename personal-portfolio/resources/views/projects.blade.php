...
@section('title', __('Portfolio') . ' - Abdul Meneam Riad')
...
<div class="container-fluid py-5 mb-5 wow fadeIn" data-wow-delay="0.1s"
        ...
        <div class="container text-center py-5 mt-5">
            <h1 class="display-2 text-white mb-4 animated slideInDown">{{ __('My Portfolio') }}</h1>
            <p class="fs-5 text-white">{{ __('Client websites, backend architectures, and physical mechatronics builds.') }}</p>
            ...
            <div class="training-item bg-white rounded h-100 d-flex flex-column shadow-sm">
              ...
              <div class="rounded-bottom border border-top-0 p-4 flex-grow-1 d-flex flex-column">
                ...
                <div class="d-flex gap-2 mt-auto">
                    @if($project->demo_url)
                        <a class="btn btn-outline-primary rounded-pill btn-sm" href="{{ $project->demo_url }}" target="_blank">{{ __('Live Demo') }}</a>
                    @endif
                    @if($project->github_url)
                        <a class="btn btn-dark rounded-pill btn-sm" href="{{ $project->github_url }}" target="_blank"><i class="fab fa-github mx-1"></i> {{ __('Code') }}</a>
                    @endif
                </div>
              </div>
            </div>
